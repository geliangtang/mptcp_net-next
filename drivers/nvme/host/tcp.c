				...
				key_serial_t pskid)
{
	...
	if (!strcmp(ctrl->ctrl.ops->name, "tcp"))
		proto = IPPROTO_TCP;
#ifdef CONFIG_MPTCP
	else if (!strcmp(ctrl->ctrl.ops->name, "mptcp"))
		proto = IPPROTO_MPTCP;
#endif
	else
		return -EINVAL;
	...
	pr_info("%s so_priority=%d nctrl->opts->tos=%d nctrl->opts->tclass=%d\n",
		__func__, so_priority, nctrl->opts->tos, nctrl->opts->tclass);
	...
	if (nctrl->opts->tclass >= 0 &&
		...
		nvme_tcp_sock_set_tclass(queue->sock->sk, nctrl->opts->tclass);
#endif
		...
		struct nvmf_ctrl_options *opts)
{
	...
	if (opts->mask & NVMF_OPT_HOST_IFACE) {
		rcu_read_lock();
		if (!dev_get_by_name_rcu(opts->net, opts->host_iface)) {
			rcu_read_unlock();
