...
void serial_test_sockmap_listen(void)
{
	...
	skel = test_sockmap_listen__open();
	if (!skel) {
		FAIL("skeleton open failed");
		return;
	}

	/* Disable MPTCP program if MPTCP is not enabled */
	if (!has_mptcp)
		bpf_program__set_autoload(skel->progs.prog_reuseport_mptcp, false);

	if (test_sockmap_listen__load(skel)) {
		FAIL("skeleton load failed");
		goto cleanup;
	}
