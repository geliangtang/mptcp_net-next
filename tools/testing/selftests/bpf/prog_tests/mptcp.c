...
static void test_mptcp_sockmap(void)
{
	...
	skel->links.mptcp_sockmap_update =
		bpf_program__attach_cgroup(skel->progs.mptcp_sockmap_update, cgroup_fd);
...
static void run_mptcp_ktls(void)
{
	...
	CHECK(has_bytes_sent(ADDR_3), "mptcp ktls", "should have bytes_sent on addr3\n");
	CHECK(has_bytes_sent(ADDR_4), "mptcp ktls", "should have bytes_sent on addr4\n");
