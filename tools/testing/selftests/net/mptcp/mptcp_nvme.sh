...
trtype="${2:-mptcp}"
path="${3:-1}"
iopolicy=${4:-"numa"} # round-robin, queue-depth
loss=${5:-0}
tls=${6:-""}
...
Usage:
	...
	$(basename "$0") [adrfam] [trtype] [path] [iopolicy] [loss] [tls]
...
run_host()
{
	...
	local extra
	if [ "${adrfam}" = "ipv6" ]; then
		extra=(--tclass="${qos}" --host-iface="${bind_iface}")
	else
		# v4map uses TOS: the connection is IPv4 over an AF_INET6
		# socket, so IP_TOS is the correct knob.
		extra=(--tos="${qos}" --host-iface="${bind_iface}")
	fi
