...
static void do_getsockopt_inq(int fd, struct msghdr *msgh,
			      unsigned int check, bool equal)
{
...
static void process_one_client(int fd, int unixfd)
{
	...
	do_getsockopt_inq(fd, &msg, expect_len - 1, true);
	...
	do_getsockopt_inq(fd, &msg, 0, true);
	...
	do {
		...
		do_getsockopt_inq(fd, &msg, expect_len - tot, false);
	...
	do_getsockopt_inq(fd, &msg, 1, true);
