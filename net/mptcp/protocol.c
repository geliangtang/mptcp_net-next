...
static struct sk_buff *tls_mptcp_recv_skb(struct sock *sk, u32 seq, u32 *off)
{
...
static u32 mptcp_get_seq(struct sk_buff *skb)
{
	return MPTCP_SKB_CB(skb)->map_seq;
}

static int tls_mptcp_read_sock(struct sock *sk, read_descriptor_t *desc,
			       sk_read_actor_t recv_actor)
{
	return __mptcp_read_sock(sk, desc, recv_actor, in_softirq());
}

static bool tls_mptcp_epollin_ready(const struct sock *sk, int target)
{
	return mptcp_epollin_ready(sk);
}

const struct tls_prot_ops tls_mptcp_ops = {
	.name		= "mptcp",
	.inq		= mptcp_inq,
	.sendmsg_locked	= mptcp_sendmsg_locked,
	.recv_skb	= tls_mptcp_recv_skb,
	.read_done	= mptcp_read_done,
	.get_seq	= mptcp_get_seq,
	.read_sock	= tls_mptcp_read_sock,
	.poll		= mptcp_poll,
	.epollin_ready	= tls_mptcp_epollin_ready,
};
#endif
