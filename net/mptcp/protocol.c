...
static bool __mptcp_move_skb(struct sock *sk, struct sk_buff *skb)
{
	...
	if (__mptcp_check_fallback(msk) &&
	    test_bit(MPTCP_AO_REPAIRED, &msk->flags)) {
		copy_len = skb->len;
...
static void mptcp_check_data_fin(struct sock *sk)
{
	...
	if (mptcp_pending_data_fin(sk, &rcv_data_fin_seq)) {
		...
		smp_mb(); /* Order SHUTDOWN, ack_seq before SOCK_DONE. */
...
static ssize_t mptcp_sendmsg_zerocopy_iter(struct sock *sk, struct msghdr *msg,
					   ...
					   u32 copy_limit)
{
	...
	if (pg_len > U16_MAX) {
		iov_iter_revert(&msg->msg_iter, pg_len - U16_MAX);
		pg_len = U16_MAX;
	}
	...
	if (pg_off + pg_len > U16_MAX) {
		size_t len = U16_MAX - pg_off;

		iov_iter_revert(&msg->msg_iter, pg_len - len);
		pg_len = len;
		if (WARN_ON_ONCE(!pg_len)) {
			put_page(pages[0]);
			return -EFAULT;
		}
	}
	...
	if (!sk_wmem_schedule(sk, pg_len + sizeof(*dfrag))) {
		kfree(dfrag);
		iov_iter_revert(&msg->msg_iter, pg_len);
		put_page(pages[0]);
		return -EAGAIN;
...
static int mptcp_sendmsg_locked(struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	while (msg_data_left(msg)) {
		...
		if (zc) {
			ret = mptcp_sendmsg_zerocopy_iter(sk, msg, ubuf,
							  copy_limit);
...
{
	const struct mptcp_sock *msk = mptcp_sk(sk);
	bool sock_done = sock_flag(sk, SOCK_DONE);
	u64 hint_val;

	/* Pair with smp_mb() in mptcp_check_data_fin(). */
	smp_rmb();

	/* Avoid races vs ack_seq updates and MPTCP_SYNC_SEQ flag. */
	mptcp_data_lock(sk);
